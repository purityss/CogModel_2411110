...
from CogModelingRNNsTutorial import bandits_LACA
...
# 构建保存路径
#save_dir = r'D:\Code\PycharmProjects\CogModelingRNNsTutorial-main\remote_test'
save_dir = r'/lustre/home/2401111678/CogModelingRNNsTutorial-main/figure'
sub = 'LACA-e-3'
# 确保路径存在
...
print("\n")
print("#################################################################")
print("--------------------Part 0---Q-agent+Bandists--------------------")
print("#################################################################")
print("\n")

'''
#@title Define an agent and an environment
print("---Define an agent and an environment---")
...
print("---Agent behavior: **One trial.**---")
...
print("---Agent behavior: **One session**---")
...
print("---Plot task structure and agent behavior---")
...
n_trials = 500  #@param
n_actions = 2
agent = bandits_LACA.LeakyActorCriticAgent(alpha_v=0.3, alpha_l=1, alpha_f=0.05, n_actions=n_actions)
environment = bandits.EnvironmentBanditsDrift(sigma=0.1, n_actions=n_actions)

# 初始化变量
choices = np.zeros(n_trials)
rewards = np.zeros(n_trials)
v_values = np.zeros(n_trials)
theta_values = np.zeros((n_trials, 1))
reward_probs = np.zeros((n_trials, n_actions))

# For each trial: Step the agent, step the environment, record everything
for trial_i in np.arange(n_trials):
    # Record environment reward probs and agent's theta values (as q)
    reward_probs[trial_i, :] = environment.reward_probs
    theta_values[trial_i] = agent.get_theta()
    v_values[trial_i] = agent.v
...
# 将v和theta合并为timeseries
timeseries = np.column_stack((v_values, theta_values))
timeseries_name = 'Value (v) and Policy Parameter (theta)'
labels = ['v', 'theta']
...
plt.tight_layout()
save_path = os.path.join(save_dir, sub, f'{sub}_p1_LACA_agent_session_0.png')
plt.savefig(save_path)
plt.show()


#@title Plot Leaky Actor-Critic agent Update Rules
print("---Plot Leaky Actor-Critic agent Update Rules---")

#agent = bandits_LACA.LeakyActorCriticAgent(alpha_v=0.3, alpha_l=1, alpha_f=0.05, n_actions=2)
plt.figure()
figs = bandits_LACA.plot_leaky_actor_critic_update_rules(agent)
# 遍历生成的图像，并保存
for i, fig in enumerate(figs):
    plt.tight_layout()
    save_path = os.path.join(save_dir, sub, f'{sub}_p2_agent_LACA_update_{i+1}.png')
...
print("---Generate synthetic data: **Whole dataset**---")
...
plt.tight_layout()
save_path = os.path.join(save_dir, sub, f'{sub}_p3_QL_diagnostics.png')
plt.savefig(save_path)
plt.show()
...
plt.tight_layout()
save_path = os.path.join(save_dir, sub, f'{sub}_p4_QL_action_similarity_to_history.png')
plt.savefig(save_path)
plt.show()
'''
...
print("\n")
print("#################################################################")
print("--------------Part 1---Fit a Classic Cognitive Model--------------")
print("#################################################################")
print("\n")
...
dataset_type = 'SyntheticLeakyActorCriticAgent'
...
if is_synthetic:
  ...
  gen_alpha_v = 0.3  #@param LACA
  gen_alpha_l = 1   #@param  LACA
  gen_alpha_f = 0.05    #@param  LACA
  n_trials_per_session = 500  #@param 200
  n_sessions = 1000  #@param 220
  sigma = .1  #@param
  environment = bandits_LACA.EnvironmentBanditsDrift(sigma=sigma)
  ...
  agent = {
      ...
      'SyntheticLeakyActorCriticAgent':bandits_LACA.LACA_Agent(gen_alpha_v, gen_alpha_l, gen_alpha_f)
      }[dataset_type]

  dataset_train, experiment_list_train = bandits_LACA.create_dataset(
      agent=agent,
      environment=environment,
      n_trials_per_session=n_trials_per_session,
      n_sessions=n_sessions)

  dataset_test, experiment_list_test = bandits_LACA.create_dataset(
      agent=agent,
      environment=environment,
      n_trials_per_session=n_trials_per_session,
      n_sessions=n_sessions)
...
#加载真实世界的实验数据 检查是否上传新数据文件 允许用户选择是否上传新数据文件
  ...
  if not _ON_COLAB:
    ...
    #LOCAL_PATH_TO_FILE = "E:/Data/rotation1_miller2018_rat/"  #@param 不要用反斜杠
    #LOCAL_PATH_TO_FILE = "E:/Data/rotation1_miller2018_rat/miller2018_rat01.npy"  # @param 不要用反斜杠'\',应指向具体的文件名
    LOCAL_PATH_TO_FILE = "/lustre/home/2401111678/CogModelingRNNsTutorial-main/rotation1_miller2018_rat/miller2018_rat01.npy"
...
plt.tight_layout()
save_path = os.path.join(save_dir, sub, f'{sub}_p5_dataset_diagnostics.png')
plt.savefig(save_path)
plt.show()
...
plt.tight_layout()
save_path = os.path.join(save_dir, sub, f'{sub}_p6_dataset_action_similarity_to_history.png')
plt.savefig(save_path)
plt.show()
...
print("---Calculate the log likelihoods for example parameters---")
...
print("---Synthesize a dataset using the fitted agent---")
...
print("---Plot internal workings (latents) of the generating fitted model (aka the Q-values)---")
...
bandits.plot_session(
    ...
    labels=[f'RL_activation {a}' for a in range(rl_activations.shape[1])])

plt.tight_layout()
save_path = os.path.join(save_dir, sub, f'{sub}_p7_RL_session_0.png')
plt.savefig(save_path)
plt.show()
...
plt.tight_layout()
save_path = os.path.join(save_dir, sub, f'{sub}_p8_RL_behavior_diagnostics.png')
plt.savefig(save_path)
plt.show()
...
plt.tight_layout()
save_path = os.path.join(save_dir, sub, f'{sub}_p9_RL_action_similarity_to_history.png')
plt.savefig(save_path)
plt.show()
...
print("\n")
print("#################################################################")
print("--------------Part 2---Fitting with RNNs--------------")
print("#################################################################")
print("\n")

#@title Set up the RNN (GRU) Model
print("Set up the RNN (GRU) Model")
...
n_steps_max = 5000 #@param 5000
optimizer = optax.adam(learning_rate=5e-3) #1e-2
...
print("Compute quality-of-fit: Held-out Normalized Likelihood")
...
#labels=[f'GRU_activation {a}' for a in range(gru_activations.shape[1])]

plt.tight_layout()
save_path = os.path.join(save_dir, sub, f'{sub}_p10_GRU_session_0.png')
plt.savefig(save_path)
plt.show()
...
#使用训练好的GRU生成一个新的数据集
print("Synthesize a dataset using the fit GRU network")
...
plt.tight_layout()
save_path = os.path.join(save_dir, sub, f'{sub}_p11_GRU_behavior_diagnostics.png')
plt.savefig(save_path)
plt.show()
...
plt.tight_layout()
save_path = os.path.join(save_dir, sub, f'{sub}_p12_GRU_action_similarity_to_history.png')
plt.savefig(save_path)
plt.show()
...
print("\n")
print("#################################################################")
print("--------------Part 3---Fitting with disRNNs--------------")
print("#################################################################")
print("\n")
...
print("Fit disRNN with no penalty at first, to get good quality-of-fit")
...
n_steps = 10000  #@param 3000 10000 50000
...
plt.tight_layout()
save_path = os.path.join(save_dir, sub, f'{sub}_p13_disRNN_Loss.png')
plt.savefig(save_path)
plt.show()
...
plt.tight_layout()
save_path = os.path.join(save_dir, sub, f'{sub}_p14_disRNN_bottlenecks.png')
plt.savefig(save_path)
plt.show()
...
for i, fig in enumerate(figs):
    plt.tight_layout()
    save_path = os.path.join(save_dir, sub, f'{sub}_p15_disRNN_update_rules_{i+1}.png')  # 保存文件路径
    fig.savefig(save_path)  # 保存图像
    #plt.close(fig)  # 关闭图像，释放内存
...
bandits.plot_session(choices=choices,
                     ...
                     timeseries_name='Network Activations',
                     labels=[f'Latent {a}' for a in range(disrnn_activations.shape[1])])
plt.tight_layout()
save_path = os.path.join(save_dir, sub, f'{sub}_p16_disRNN_latents_choices_session_0.png')
plt.savefig(save_path)
plt.show()
...
print('Synthesize a dataset using the fit network')
...
plt.tight_layout()
save_path = os.path.join(save_dir, sub, f'{sub}_p17_disRNN_behavior_diagnostics.png')
plt.savefig(save_path)
plt.show()
...
plt.tight_layout()
save_path = os.path.join(save_dir, sub, f'{sub}_p18_disRNN_action_similarity_to_history.png')
plt.savefig(save_path)
plt.show()
...
print("\n")
print("#################################################################")
print("--------------Part 3---Fitting with Hybrid RNNs--------------")
print("#################################################################")
print("\n")
...
print("Save out latent variables from the network")
...
bandits.plot_session(choices=choices,
                     ...
                     labels = [f'Hybrid RNN Values {a}' for a in range(hybrnn_values.shape[1])])

plt.tight_layout()
save_path = os.path.join(save_dir, sub, f'{sub}_p19_HybridRNN_session_0.png')
plt.savefig(save_path)
plt.show()
...
plt.tight_layout()
save_path = os.path.join(save_dir, sub, f'{sub}_p20_Q-values.png')
plt.savefig(save_path)
plt.show()
'''
...
plt.tight_layout()
save_path = os.path.join(save_dir, sub, f'{sub}_p21_HybridRNN_action_similarity_to_history.png')
plt.savefig(save_path)
plt.show()
