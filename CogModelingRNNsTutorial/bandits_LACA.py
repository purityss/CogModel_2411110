...
############################################
# LeakyActorCriticAgent #  2024.10.8 by zss
############################################
...
class LeakyActorCriticAgent:
  """
  Leaky Actor-Critic Agent with theta_left and theta_right combined as theta = theta_left - theta_right.
  Attributes:
      alpha_v: learning rate for the critic (value function)
      alpha_l: learning rate for the actor (policy)
      alpha_f: forgetting rate for the actor's policy
      n_actions: number of possible actions (fixed at 2: left and right)
      v: value function (critic)
      theta_left: policy parameter for left action
      theta_right: policy parameter for right action
  """

  def __init__(self, alpha_v=0.1, alpha_l=0.01, alpha_f=0.05, n_actions=2):
    if n_actions != 2:
      raise ValueError("This implementation only supports n_actions=2 for left and right actions.")

    self.alpha_v = alpha_v  # Critic learning rate
    self.alpha_l = alpha_l  # Actor learning rate
    self.alpha_f = alpha_f  # Forgetting rate
    self.n_actions = n_actions
    self.v = 0.0  # Critic value starts at 0
    self.theta_left = 0.0  # Policy parameter for left action
    self.theta_right = 0.0  # Policy parameter for right action

  def get_theta(self):
    """Calculate the policy parameter theta as theta_left - theta_right."""
    return self.theta_left - self.theta_right

  def softmax(self, x):
    """Compute the softmax of a scalar."""
    exp_x = np.exp(x)
    return exp_x / np.sum(exp_x)

  def get_choice_probs(self):
    """Compute the action choice probabilities using softmax over theta."""
    theta = self.get_theta()
    prob_left = 1 / (1 + np.exp(theta))  # Sigmoid for two actions
    return np.array([prob_left, 1 - prob_left])

  def get_choice(self):
    """Sample a choice based on the current policy probabilities."""
    choice_probs = self.get_choice_probs()
    return np.random.choice(self.n_actions, p=choice_probs)

  def update_critic(self, reward):
    """Update the critic (value function) using the given rule."""
    self.v = (1 - self.alpha_v) * self.v + self.alpha_v * reward

  def update_actor(self, action, reward):
    """Update the actor (policy parameters) using the policy gradient rules from the paper."""
    pi = self.get_choice_probs()
    td_error = reward - self.v  # Temporal difference error, (r_t - V_t)

    # Update for the chosen action (left or right)
    if action == 1:  # Chose left
      self.theta_left = (1 - self.alpha_f) * self.theta_left + self.alpha_l * td_error * (1 - pi[1])
      self.theta_right = (1 - self.alpha_f) * self.theta_right - self.alpha_l * td_error * pi[0]
    else:  # Chose right
      self.theta_right = (1 - self.alpha_f) * self.theta_right + self.alpha_l * td_error * (1 - pi[0])
      self.theta_left = (1 - self.alpha_f) * self.theta_left - self.alpha_l * td_error * pi[1]
    #print(f"Action: {action}, Reward: {reward}, TD Error: {td_error}")

  def update(self, choice: int, reward: int):
    """Update both critic and actor after one step of the task."""
    self.update_critic(reward)
    self.update_actor(choice, reward)

  def new_sess(self):
    """Reset the agent for the beginning of a new session."""
    self.v = 0.0  # Reset value function
    self.theta_left = 0.0  # Reset policy parameter for left action
    self.theta_right = 0.0  # Reset policy parameter for right action


class LACA_Agent(LeakyActorCriticAgent):
  """This agent is a wrapper of AgentQ with only alpha and beta parameters."""

  def __init__(self, alpha_v: float, alpha_l: float, alpha_f: float,n_actions: int = 2):
    super(LACA_Agent, self).__init__(
        alpha_v, alpha_l, alpha_f, n_actions=n_actions)


'''         
class LeakyActorCriticAgent:
  """
  Leaky Actor-Critic Agent with a single theta value for policy, similar to AgentQ.
  Attributes:
      alpha_v: learning rate for the critic (value function)
      alpha_l: learning rate for the actor (policy)
      alpha_f: forgetting rate for the actor's policy
      ...
      v: value function (critic)
      theta: policy parameter for action selection
  """

  def __init__(self, alpha_v=0.1, alpha_l=0.01, alpha_f=0.05, n_actions=2):
    self.alpha_v = alpha_v  # Critic learning rate
    self.alpha_l = alpha_l  # Actor learning rate
    self.alpha_f = alpha_f  # Forgetting rate for theta
    self.n_actions = n_actions
    self.theta_init = 0.0
    self.new_sess()
  ...
  def new_sess(self):
    """Reset the agent for the beginning of a new session."""
    self.v = 0.0  # Reset value function
    self.theta = np.ones(self.n_actions) * self.theta_init  # Reset theta

  def get_choice_probs(self) -> np.ndarray:
    """Compute the choice probabilities as softmax over theta."""
  ...
  def update_actor(self, choice: int, reward: int):
    """Update the actor (policy) using the policy gradient rules."""
    choice_probs = self.get_choice_probs()
    td_error = reward - self.v  # Temporal difference error (reward - value)

    # Update theta for the chosen action
    self.theta[choice] = (1 - self.alpha_f) * self.theta[choice] + self.alpha_l * td_error * (1 - choice_probs[choice])

    # Apply forgetting and update theta for non-chosen actions
    for a in range(self.n_actions):
      if a != choice:
        self.theta[a] = (1 - self.alpha_f) * self.theta[a] - self.alpha_l * td_error * choice_probs[a]
  ...
  @property
  def get_theta(self):
    """Returns a copy of the current theta values."""
...
#############################################################
# Plot LeakyActorCriticAgent Update Rules #  2024.10.9 by zss
#############################################################

def plot_leaky_actor_critic_update_rules(agent, n_steps=20):
  """
  Visualize value function (V) and policy parameters (Theta) update rules
  for the Leaky Actor-Critic Agent, with a separate colorbar for V values.

  Args:
  agent: An instance of the LeakyActorCriticAgent class.
  n_steps: Number of state bins (discrete steps for theta values).
  Returns:
  figs: List of matplotlib figures for V and Theta visualizations, with an additional figure for colorbar.
  """

  # Define the reward range and theta_bins
  v_bins = np.linspace(0, 1, n_steps)  # V should range between 0 and 1
  theta_bins = np.linspace(-2, 2, n_steps)  # Simulating theta values between -2 and 2
  lim_theta = 2  # Limit for Theta plotting

  # Prepare a list to hold all figures
  figs = []

  # Plot for V (value function) updates
  fig_v, axes_v = plt.subplots(1, 2, figsize=(12, 6))  # Two subplots: Unrewarded and Rewarded
  titles_v = ['Unrewarded', 'Rewarded']

  for reward_index, reward in enumerate([0, 1]):  # Unrewarded (0) and Rewarded (1)
    delta_v = np.zeros(len(v_bins))

    # Start with an initial V
    agent.v = 0.0
    for v_i, v_value in enumerate(v_bins):
      agent.v = v_value
      # Update V based on the reward
      agent.update_critic(reward)
      delta_v[v_i] = agent.v  # Store the updated value of V

    # Plot V updates for Unrewarded and Rewarded
    ax_v = axes_v[reward_index]
    ax_v.plot(v_bins, delta_v, color='green')
    ax_v.plot([0, 1], [0, 1], '--', color='grey')  # Diagonal dashed line
    ax_v.plot([0, 0], [-1, 1], color='black')  # Vertical line at x=0
    ax_v.plot([-1, 1], [0, 0], color='black')  # Horizontal line at y=0
    ax_v.set_title(f'V (Value Function) Updates: {titles_v[reward_index]}')
    ax_v.set_xlabel('Initial V')
    ax_v.set_ylabel('Updated V')
    ax_v.set_xlim(0, 1)
    ax_v.set_ylim(0, 1)

  plt.tight_layout()
  figs.append(fig_v)  # Append V plot to figs list

  # Plot for Theta updates (for each action and reward condition)
  fig_theta, axes_theta = plt.subplots(1, 4, figsize=(20, 6))  # Four subplots: Left/Right + Unrewarded/Rewarded
  titles_theta = ['Left, Unrewarded', 'Left, Rewarded', 'Right, Unrewarded', 'Right, Rewarded']

  # Create a colormap for V values
  cmap = cm.get_cmap('viridis')
  norm = plt.Normalize(0, 1)  # Normalize V values between 0 and 1

  for obs_index in range(4):
    # Determine action and reward based on subplot index
    #choice = 0 if obs_index < 2 else 1  # Left choice for first two subplots, Right choice for last two
    #reward = 0 if obs_index % 2 == 0 else 1  # Unrewarded for even indices, Rewarded for odd indices
    choice = 1 if obs_index < 2 else 0  # Left choice for first two subplots, Right choice for last two
    reward = 0 if obs_index % 2 == 0 else 1  # Unrewarded for even indices, Rewarded for odd indices

    # Plot multiple lines for different V values
    ax_theta = axes_theta[obs_index]
    v_values = np.linspace(0, 1, 10)  # Create 10 different V values for lines
    for v in v_values:
      delta_theta = np.zeros(len(theta_bins)) #
      agent.v = v  # Set current V for this line

      for theta_i, theta_value in enumerate(theta_bins):
        # Reset theta_left and theta_right
        agent.theta_left = theta_value
        agent.theta_right = 0.0  # Keep one side fixed to observe changes

        # Update the actor based on the action, reward, and V
        agent.update_actor(choice, reward)

        # Calculate theta as theta_left - theta_right
        delta_theta[theta_i] = agent.get_theta()

      # Plot the line for this V value
      ax_theta.plot(theta_bins, delta_theta, color=cmap(norm(v)))

    # Add diagonal dashed line and axis lines
    ax_theta.plot([-2, 2], [-2, 2], '--', color='grey')  # Diagonal dashed line
    ax_theta.plot([0, 0], [-2, 2], color='black')  # Vertical line at x=0
    ax_theta.plot([-2, 2], [0, 0], color='black')  # Horizontal line at y=0
    ax_theta.set_title(f'{titles_theta[obs_index]}')
    ax_theta.set_xlabel('Initial Theta (theta_left - theta_right)')
    ax_theta.set_ylabel('Updated Theta')
    ax_theta.set_xlim(-lim_theta, lim_theta)
    ax_theta.set_ylim(-lim_theta, lim_theta)

  plt.tight_layout()
  figs.append(fig_theta)  # Append Theta plot to figs list

  # Create a separate figure for colorbar
  fig_colorbar, ax_colorbar = plt.subplots(figsize=(1, 5))  # Separate figure for colorbar
  sm = plt.cm.ScalarMappable(cmap=cmap, norm=norm)
  sm.set_array([])  # Only needed for colorbar
  fig_colorbar.colorbar(sm, cax=ax_colorbar, label='V value')
  figs.append(fig_colorbar)  # Append colorbar plot to figs list
...
Agent = Union[LeakyActorCriticAgent, AgentNetwork]
