...
class DatasetRNN():
  """Holds a dataset for training an RNN, consisting of inputs and targets.
     管理RNN训练所需的数据集。该类存储输入（xs）和目标输出（ys），并按批次提供数据，以便用于训练RNN模型
     Both inputs and targets are stored as [timestep, episode, feature] [时间步的维度，训练样本的维度，每个时间步的特征维度]
  ...
  def __init__(self,
    ...
    Args:
      ...
      batch_size: The size of the batch (number of episodes) to serve up each
        ...
        will be served
        每次调用 next() 时，返回的批次大小，即所包含的episode数量。如果未指定，则默认使用所有episode
    """
    ...
    # Is the number of episodes divisible by the batch size? 检查批次大小是否能被episode数整除
  ...
  def __next__(self) -> Tuple[chex.Array, chex.Array]:
    ...
    定义了如何从数据集中返回下一个批次的数据。在每次调用时，它根据当前的索引 _idx，从数据集中获取一个批次的输入 (x) 和目标 (y)，并将索引更新到下一个批次的位置。
    Returns:
    ...
    # Check that we're not trying to overshoot the size of the dataset  检查是否超出数据集大小
    assert end <= self._dataset_size

    # Update the index for next time 更新索引
...
def get_initial_state(make_network: Callable[[], hk.RNNCore],
  ...
  #random_key = jax.random.PRNGKey(np.random.randint(0, 2**16))
