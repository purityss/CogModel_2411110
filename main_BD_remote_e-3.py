...
#加载真实世界的实验数据 检查是否上传新数据文件 允许用户选择是否上传新数据文件
  ...
  if not _ON_COLAB:
    ...
    #LOCAL_PATH_TO_FILE = "E:/Data/rotation1_Dezfouli2019_patients/BD-healthy.hdf5"  # @param 不要用反斜杠'\',应指向具体的文件名
    LOCAL_PATH_TO_FILE = "/lustre/home/2401111678/CogModelingRNNsTutorial-main/rotation1_Dezfouli2019_patients/BD-healthy.hdf5"
  ...
  # 加载数据
  with h5py.File(FNAME_, 'r') as f:
      action = f['action'][:].astype(np.int32)
      reward = f['reward'][:].astype(np.float32)

  # 构造 xs (动作和奖励) 和 ys (下一步的选择)
  xs = np.stack((action, reward), axis=-1)
  ys = np.roll(action, shift=-1, axis=0)[..., np.newaxis]

  # 使用 format_into_datasets 函数进行划分
  dataset_train, dataset_test = rat_data.format_into_datasets(xs, ys, rnn_utils.DatasetRNN)
...
# 输出该会话的选择
#print("Choices for session", sess_i, "are:", choices)
#print("Rewards for session", sess_i, "are:", rewards)
